...
COMMIT_URL="${REPOSITORY_BASE_URL}/commit"
COMPARE_URL="${REPOSITORY_BASE_URL}/compare"

# ---- AUTH ----
[ -n "$GH_TOKEN" ] && echo "[$(date)][AUTH]: GH_TOKEN found '${GH_TOKEN}'" || {
  echo "[$(date)][AUTH]: Error! release needs GH_TOKEN, use export GH_TOKEN='your Token'."
  exit 1
}
AUTH_RESPONSE=$(curl --silent -I -X GET  -H "Authorization: token ${GH_TOKEN}" "https://api.github.com"| awk '/^HTTP/{print $2}' \
| tr ',' '\n' | sort -V)
...
# ---- INIT DATA ----
git fetch --tag https://"${GH_TOKEN}":"${GH_TOKEN}"@github.com/"${ORGANIZATION}"/"${REPOSITORY}"
